# memtree v1 Implementation Plan
...
**Goal:** Build memtree v1 — a TypeScript MCP server that exposes surgical recall tools (`memtree.read`, `memtree.grep`, `memtree.compose`, `memtree.search`, `memtree.neighbors`, `memtree.path_to_root`, `memtree.recent`). The agent uses these tools directly; outputs are written to SQLite at call time.

**Architecture:** Single Node process: MCP server + background walker coordinator. The memtree-backed tool handlers write to SQLite directly via `better-sqlite3` prepared statements. Walkers (`filter`, `staleness_marker`, `pruner`) run on `setInterval` with error boundaries and a startup sweep. No socket server, no hook, no threads in v1. (PostToolUse hook and socket ingestion are v1.1.)
...
│   │   └── tools/
│   │       ├── search.ts          # memtree.search (keyword only in v1)
│   │       ├── neighbors.ts       # memtree.neighbors (depth cap 5)
│   │       ├── path-to-root.ts    # memtree.path_to_root
│   │       ├── recent.ts          # memtree.recent
│   │       ├── read.ts            # memtree.read (tree-sitter chunks + 200-line fallback)
│   │       ├── grep.ts            # memtree.grep (ripgrep + child file_chunk nodes)
│   │       └── compose.ts         # memtree.compose (BFS expand, score, budget-pack)
│   └── eval/
│       ├── run.ts
...
```json
{
  ...
  "workspaces": ["mcp"],
  "scripts": {
    "build": "bun run --filter @memtree/mcp build",
    "test": "bun test --filter @memtree/mcp",
    "dev": "bun run --filter @memtree/mcp dev"
  }
}
```
...
export function getNode(db: Database.Database, id: string): MemtreeNode | null {
  return (db.prepare('SELECT * FROM nodes WHERE id = ?').get(id) as MemtreeNode | undefined) ?? null;
}
...
export function getNodeBySourceUri(db: Database.Database, sourceUri: string): MemtreeNode | null {
  return (db.prepare(
...
export function getNodeByContentHash(db: Database.Database, hash: string): MemtreeNode | null {
  return (db.prepare(
...
export function getPendingNodes(db: Database.Database, limit = 100): MemtreeNode[] {
  return db.prepare(
...
export function insertEdge(db: Database.Database, edge: Omit<MemtreeEdge, 'created_at'>): void {
  db.prepare(`
...
export function getEdgesFrom(db: Database.Database, srcId: string): MemtreeEdge[] {
  return db.prepare('SELECT * FROM edges WHERE src_id = ?').all(srcId) as MemtreeEdge[];
}
```
...
> **Skip for v1.** This entire task has been moved to `docs/superpowers/plans/2026-05-17-memtree-v1.1.md`.
>
> In v1 there is no Unix socket ingestion server. The memtree-backed tool handlers
> (`memtree.read`, `memtree.grep`) write to SQLite directly via `better-sqlite3`.
...
export function runFilterWalker(db: Database.Database, config: MemtreeConfig): void {
...
export function runStalenessWalker(db: Database.Database, _config: MemtreeConfig): void {
...
export function runPrunerWalker(db: Database.Database, config: MemtreeConfig): void {
  const now = Date.now();
...
export class WalkerCoordinator {
  ...
  startupSweep(db: Database.Database, config: MemtreeConfig): void {
  ...
  start(db: Database.Database, config: MemtreeConfig): void {
...
export function getPathToRoot(db: Database.Database, nodeId: string): MemtreeNode[] {
  const path: MemtreeNode[] = [];
  let current = db.prepare('SELECT * FROM nodes WHERE id = ?').get(nodeId) as MemtreeNode | undefined;

  while (current) {
    path.push(current);
    if (!current.parent_id) break;
    current = db.prepare('SELECT * FROM nodes WHERE id = ?').get(current.parent_id) as MemtreeNode | undefined;
  }
...
## Task 11: memtree.read

**Files:**
- Create: `mcp/src/tools/read.ts`
- Test: `mcp/src/tools/read.test.ts`

`memtree.read` reads a file, chunks it by symbol via tree-sitter (or 200-line windows as fallback), stores chunks as `file_chunk` nodes, returns the requested slice. If the file's `mtime` is unchanged, returns cached nodes.
...
git commit -m "feat(tools): memtree.read — tree-sitter chunking + 200-line fallback + mtime cache"
```

---

## Task 12: memtree.grep

**Files:**
- Create: `mcp/src/tools/grep.ts`
- Test: `mcp/src/tools/grep.test.ts`

`memtree.grep` shells out to system `rg`, stores the aggregate result as a `tool_output` node, and creates child `file_chunk` nodes for each matched file.
...
git commit -m "feat(tools): memtree.grep — ripgrep integration + tool_output node storage"
```

---

## Task 13: memtree.compose
...
git commit -m "feat(tools): memtree.compose — BFS expand, score, budget-pack, raw/outline formats"
```
...
import { memtreeRead } from './tools/read';
import { memtreeGrep } from './tools/grep';
import { memtreeCompose } from './tools/compose';
import type { Filters } from './store/types';
...
function checkSystemDeps(): void {
  ...
  for (const { cmd, name } of deps) {
    try { execSync(cmd, { stdio: 'pipe' }); } catch {
      process.stderr.write(`memtree: missing system dep: ${name} — install via brew/apt\n`);
      process.exit(1);
    }
  }
}
...
function checkPlatform(): void {
  ...
  if (!supported.some(([p, a]) => p === platform && a === arch)) {
    process.stderr.write(`memtree: unsupported platform: ${platform}-${arch}\n`);
    process.exit(1);
  }
}
...
async function main() {
  ...
  const cwd = process.env.MEMTREE_CWD ?? process.cwd();
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      {
        name: 'memtree_search',
        description: 'Keyword search across the memtree store. Only mode="keyword" is supported in v1.',
        inputSchema: {
          type: 'object',
          properties: {
            query: { type: 'string' },
  ...
  server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    try {
      switch (name) {
        ...
        case 'memtree_read': {
          const result = await memtreeRead(db, config, {
...
main().catch(e => {
  process.stderr.write(`memtree startup error: ${e}\n`);
  process.exit(1);
});
```
...
- Create: `skills/using-memtree/SKILL.md`
- Create: `README.md`
...
{
  "name": "memtree",
  "description": "Tree/graph-based persistent context store with surgical recall. Exposes memtree.read, memtree.grep, memtree.compose, memtree.search, memtree.neighbors for budget-bounded recall.",
...
- [ ] **Step 2: Create `skills/using-memtree/SKILL.md`**
...
```bash
git add .claude-plugin/plugin.json skills/using-memtree/SKILL.md README.md
git commit -m "feat: plugin manifest, using-memtree skill, README"
```
...
const REPO = 'joeblackwaslike/memtree';
...
if (!SUPPORTED.includes(TRIPLE)) {
  process.stderr.write(`memtree: unsupported platform ${TRIPLE}\n`);
  process.exit(1);
}
...
if (existsSync(STAMP)) {
  process.stdout.write(`memtree: native deps already installed for ${TRIPLE}\n`);
  process.exit(0);
}

mkdirSync(NATIVE_DIR, { recursive: true });

const ARCHIVE_NAME = `memtree-native-${TRIPLE}.tar.gz`;
...
async function main() {
  process.stdout.write(`memtree: downloading native deps for ${TRIPLE} (v${VERSION})...\n`);
  ...
  if (actual !== expected) {
    process.stderr.write(`memtree: checksum mismatch for ${ARCHIVE_NAME}\n`);
  ...
  process.stdout.write(`memtree: native deps installed to ${NATIVE_DIR}\n`);
}

main().catch(e => {
  process.stderr.write(`memtree postinstall failed: ${e}\n`);
  process.exit(1);
});
```
...
for TRIPLE in $TRIPLES; do
  ...
  # Bundle into archive
  ARCHIVE="$DIST/memtree-native-$TRIPLE.tar.gz"
...
gh release create "v$VERSION" "$DIST"/memtree-native-*.tar.gz "$DIST"/memtree-native-*.sha256 \
  --repo "$REPO" \
  --title "v$VERSION" \
  --generate-notes
```
...
export async function runSecurityCases(db: Database.Database): Promise<SecurityResult[]> {
  ...
  // Secret file paths must be rejected by memtreeRead (throws "Path rejected by denylist")
...
async function main() {
  ...
  // ── Schema rejection ────────────────────────────────────────────────────────
  console.log('\n=== Schema Rejection ===');
  const { memtreeCompose } = await import('../src/tools/compose');
  try {
    await memtreeCompose(db, { node_ids: ['x'], budget_tokens: 100, format: 'mixed' });
...
=== Latency Cases ===
  ✓ memtree.read cached p50: p50=X.Xms p95=X.Xms (threshold: 50ms)
...
| §4 memtree-backed tools, cache invalidation, compose algorithm | Tasks 11–13 |
...
| §8 Plugin manifest, using-memtree skill (no hook in v1) | Task 16 |
| §11 Acceptance: security, latency, schema rejection, graceful degradation | Task 18 |

**v1.1 explicitly deferred (no tasks):** passive PostToolUse hook; `ingest.ts`; `projects.tsv`; `capture.sh`; `embedding_indexer`, `summarizer`, `dedupe` walkers; `nodes_vec` stays empty; `memtree.bash`; `mode='hybrid'`/`'semantic'`; bundled hook binary; `updatedToolOutput` rewriting.

**Placeholder scan:** None found. All code blocks are complete.

**Type consistency:** `MemtreeNode`, `MemtreeEdge`, `Filters`, `ComposeManifest`, `MemtreeConfig` defined in Task 2 and used consistently across Tasks 6–14.
