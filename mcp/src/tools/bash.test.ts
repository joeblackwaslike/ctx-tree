...
import { ctxTreeBash } from './bash.js';
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'ctx-tree-bash-test-'));
...
describe('ctxTreeBash', () => {
  test('denylist rejects env command', async () => {
    await expect(
      ctxTreeBash(store, trustedConfig, { command: 'env' })
    ).rejects.toBeInstanceOf(McpError);

    try {
      await ctxTreeBash(store, trustedConfig, { command: 'env' });
    } catch (err) {
  ...
  test('denylist rejects printenv HOME', async () => {
    await expect(
      ctxTreeBash(store, trustedConfig, { command: 'printenv HOME' })
    ).rejects.toBeInstanceOf(McpError);

    try {
      await ctxTreeBash(store, trustedConfig, { command: 'printenv HOME' });
    } catch (err) {
      expect(err).toBeInstanceOf(McpError);
      expect((err as McpError).message).toContain('rejected by denylist');
    }
  ...
  test('trustedExecution=false (default) throws documented error', async () => {
    ...
    try {
      await ctxTreeBash(store, untrustedConfig, { command: 'echo hello' });
  ...
  test('normal command with trustedExecution=true stores a DB row', async () => {
    const result = await ctxTreeBash(store, trustedConfig, { command: 'echo hello world' });
  ...
  test('output redaction removes secrets before storage', async () => {
    ...
    const result = await ctxTreeBash(store, trustedConfig, {
      command: `echo '${awsKey}'`,
    });
