...
memtree installs 10 Claude Code hooks automatically. You don't configure them — they fire on every relevant tool call and session event.
...
| `PreToolUse` | `pretooluse-redirect.mjs` | Blocks Read/Grep/WebFetch/Bash(grep/cat) → memtree equivalent |
...
| `PreToolUse` | `pretooluse-proxy.mjs` | Soft advisory mode (when `~/.memtree/proxy-mode` flag is set) |
...
2. Injects a `hookSpecificOutput.additionalContext` message telling Claude which memtree tool to use instead and with what parameters

This ensures raw tool output never enters the context window, even if Claude forgets to use a memtree tool directly.
...
2. Runs `memtree_search` against the store
...
After a `WebSearch` completes, queues the result URLs for background browsing via `memtree_browse`. The URLs are fetched and stored as `web_chunk` nodes asynchronously — they're available for recall in subsequent turns or future sessions without consuming context now.

---

## Proxy mode

Set `~/.memtree/proxy-mode` (any content) to activate soft advisory mode via `pretooluse-proxy.mjs`. In this mode, the hook **nudges without blocking** — it injects a suggestion to use the memtree equivalent but lets the native tool call proceed. Useful for debugging or gradual adoption.
